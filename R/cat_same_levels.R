...
for (i in 1:(dim(df)[1])){
if(sum(df$ICmin[i]>df$ICmax)>1) texto = c(texto,paste0(" *  ",df$Variável[i]," é maior que ",printvetor(df$Variável[df$ICmin[i]>df$ICmax]),"  \n.",sep="")) else
  if(sum(df$ICmin[i]>df$ICmax)==1) texto = c(texto,paste0(" *  ",df$Variável[i]," é maior que ",df$Variável[df$ICmin[i]>df$ICmax],"  \n.", sep="")) else
    if(i==dim(df)[1]) texto = c(texto,paste0(" *  ",df$Variável[i]," não é maior que nenhum.  \n",sep="")) else {
      texto=c(texto, paste0(" *  ",printvetor(df$Variável[i:dim(df)[1]])," não são maiores que nenhum.  \n",sep="")); break}}
