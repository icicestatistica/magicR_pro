...
texto=list("Podemos avaliar esta tabela comparando os intervalos de confiança de cada ítem. Caso os intervalos de confiança de dois ítens se sobreponham, isso significa que não rejeitamos a diferença entre as proporções nesses dois ao nível de 5% de significância. Portanto, interpretamos da seguinte forma:","\n")
for (i in 1:(dim(df)[1])){
if(sum(df$ICmin[i]>df$ICmax)>1) texto = list.append(texto," * ",df$Variável[i]," é maior que ",printvetor(df$Variável[df$ICmin[i]>df$ICmax]),".","\n") else
  if(sum(df$ICmin[i]>df$ICmax)==1) texto = list.append(texto," * ",df$Variável[i]," é maior que ",df$Variável[df$ICmin[i]>df$ICmax],".","\n") else
    if(i==dim(df)[1]) texto = list.append(texto," * ",df$Variável[i]," não é maior que nenhum.","\n") else {
      texto=list.append(texto, paste(" * ",printvetor(df$Variável[i:dim(df)[1]])," não são maiores que nenhum.","\n")); break}}
texto=list.append(texto, "É possível visualizar esses resultados no gráfico a seguir:","\n")
