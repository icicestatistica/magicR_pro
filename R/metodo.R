metodo = function(analises){
  ...
  final = NULL
  contagem = data.frame("numeric"=F,"ordinal"=F,"factor"=F,"catsame"=F,"toumw"=F,"anovaoukruskall"=F,"cc"=F,"correl"=F,"wilc"=F,"fried"=F,"mcnem"=F,"qcoch"=F)
  ...
  for (i in sess) {
    ...
    if (cont[13]>0) {
      if (contagem$wilc==F) {desc = c(desc,paste0("\n+ Comparamos as variáveis  ",printvetor(unique(parte$Nome2[parte$tipo %in% c("wilc")]),aspas=F)," em diferentes ",printvetor(unique(parte$Nome1[parte$tipo %in% c("wilc")]),aspas=F)," através do teste não paramétrico de Wilcoxon [@myles_1973]. Em conjunto com o p-valor, foi calculado o tamanho de efeito r [@maciej_2014], com a seguinte interpretação: r > 0.1 como efeito pequeno, r > 0.3 efeito médio e r > 0.5 grande." )); contagem$wilc=T} else {
        desc = c(desc,paste0("\n+ Comparamos as variáveis  ",printvetor(unique(parte$Nome2[parte$tipo %in% c("wilc")]),aspas=F)," em diferentes  ",printvetor(unique(parte$Nome1[parte$tipo %in% c("wilc")]),aspas=F)," através do teste não paramétrico de Wilcoxon em conjunto com o tamanho de efeito r." ))}}  
