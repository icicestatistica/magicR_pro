metodo = function(analises){
  ...
  final = NULL
  contagem = data.frame("numeric"=F,"ordinal"=F,"factor"=F,"catsame"=F,"toumw"=F,"anovaoukruskall"=F,"cc"=F,"correl"=F,"t_par"=F,"fried"=F,"mcnem"=F)
  ...
  for (i in sess) {
    ...
    cont=table(factor(parte$tipo,levels=c("numeric","ordinal","factor","catsame","t","mw","aov","kw","cc","correl","t_par","fried","mcnem")))>0
