...
grafico_categorica <- function(var,nome, niveis, cor, ordenar){
  ...
  ggplot() + geom_bar(aes(x=var,y=Freq),fill=cor,stat="identity")  + 
    ylim(0,max(table(var))*1.1)+theme_clean()  + ylab("") + xlab("") + ggtitle(paste(vetor_comsep(nome,12)," (n=,length(na.omit(var)),")",collapse=""))+ geom_text(aes(x=var,y=Freq),label=tab$perc,vjust=-0.5) +
    theme(
