escolha_summary_para_juntar <- function(x,nomesx,tipox,niveisx,nas,teste,grafico,cor,bins,dig){
if (tipox=="factor") {resulta=desc_uni_categorica(x,nomesx,eval(parse(text=niveisx)),nas,T,T,F,teste,grafico,cor,dig)
  ...
  if (tipox=="ordinal") {resulta=desc_uni_categorica(x,nomesx,eval(parse(text=niveisx)),nas,T,F,F,teste,grafico,cor,dig)
                        ...
                        tabela=resulta$tabela
                        grafico=resulta$grafico} else
    {resulta=desc_uni_continua(x,nomesx,bins,teste,grafico,cor,digitos)
     texto=resulta$texto
     tabela=NULL
...
get_summary_2 <- function(x,nomesx,tipox,niveisx,nas,teste,grafico,cor,bins,dig){
  cont=0
  xdim <- dim(x)[2]
  cont=cont+1
...
resultados = get_summary_2(x,nomesx,tipox,niveisx,nas,teste,grafico,cor,bins,dig)
