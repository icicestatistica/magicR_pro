...
get_summary_2 <- function(x,nomesx,tipox,niveisx,nas,teste,grafico,cor,dig){
  ...
  xdim <- dim(x)[2]
  cont=cont+1
  result <- escolha_summary_para_juntar(x[,1],nomesx[1],tipox[1],niveisx[1],nas,teste,grafico,cor,dig)$result
  if (xdim>1){
  for (i in 2:xdim){
    cont=cont+1
    result <- rbind(result,escolha_summary_para_juntar(x[,i],nomesx[i],tipox[i],niveisx[i],nas,teste,grafico,cor,dig)$result)}}
