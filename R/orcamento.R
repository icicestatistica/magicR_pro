...
for (i in 1:n_analises){
  ...
  if(sum(matanalises[i,-c(1:3,16)])>0) {descricaoanalises <- c(descricaoanalises,paste(" + **", matanalises[i,1],"**, com testes de comparação (podendo ser Qui-quadrado, Exato de Fisher, Teste de correlação, Teste-t, Mann-Whitney, Anova ou Kruskall Wallis, a depender da natureza e características dos dados) e suas devidas análises post-hoc e, quando necessário, tamanho de efeito. As ",sum(matanalises[i,-c(1:3)])," variáveis utilizadas na análise serão: ",matanalises$Variáveis,"; \n",sep=""))}}
