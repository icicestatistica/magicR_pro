  ...
  if(tipox=="numeric" | tipox=="ordinal") {
    ...
    if(tipoy=="factor") {
      if(length(niveisy)==2) 
           ...
           if(res$sup==F | tipox=="ordinal") {res=mann(y,as.numeric(unlist(x)),nomey,nomex,niveisy,dig,F,excluirtotal,cor,idioma,transform_ord) ; mw=1} else {t=1}
           result=res$resul
           tex=res$texto
  ...
  if(tipox=="factor") {
    if(tipoy=="numeric" | tipoy=="ordinal") {
      ...
      if(length(niveisx)==2)
        ...
        if(res$sup==F | tipoy=="ordinal") {res=mann(x,as.numeric(unlist(y)),nomex,nomey,niveisx,dig,T,excluirtotal,cor,idioma,transform_ord) ; mw=1} else {t=1}
                        result=res$resul
                        tex=res$texto
                        grafico=res$grafico} else
