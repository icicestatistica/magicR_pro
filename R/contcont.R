contcont <- function(y,x,nomey,nomex,dig=2,cor="cyan4",idioma="PT",ordinalx='auto',ordinaly='auto'){
    method = "spearman"
    x = unlist(x)
    y = unlist(y)
    d = data.frame(x, y)
    if(ordinalx=='auto') ordinalx=ifelse(length(table(d$x))>10,F,T)
    ...
    if (str1 == "N/A" || str2 == "N/A") 
        sup = c(" A suposição de normalidade das amostras não pode ser verificada pelo teste de Shapiro-Wilk, uma vez que o tamanho de pelo menos uma das amostras foi menor que 3 ou maior que 5000. Por este motivo, utilizamos o teste não paramétrico de correlação de spearman ao invés da correlação de pearson.") else {
        if (str1$p.value >= 0.05 & str2$p.value >= 0.05) {
            sup = c(" A suposição de normalidade das amostras foi verificada através do teste de Shapiro-Wilk, que com p-valor maior que 0.05 para as duas amostras não rejeitou a normalidade das distribuições (", 
                nomex, " - W=", round(str1$statistic, 2), 
                ", p-valor=", pvalor(str1$p.value), ", ", 
                nomey, " - W=", round(str2$statistic, dig), 
                ", p-valor=", pvalor(str2$p.value), "), atendendo a suposição do teste.")
            method = "pearson"
        }
        if (str1$p.value >= 0.05 & str2$p.value < 0.05) 
    ...
    a = cor.test(as.numeric(y), as.numeric(x), method = method, exact = F)
    if (method == "spearman") {
        letra = "g"
        greg = "$\\rho$"
        calcIC = " (calculado via bootstrap (BCa - adjusted bootstrap percentile, com 1000 reamostragens) "
        bootc = boot(data.frame(x=as.numeric(x),y=as.numeric(y)), bootcor, R = 2000, met = "spearman")
        ac = boot.ci(bootc, type = "bca")
        IC = paste0("(", round(ac$bca[4], dig), " ,", 
            round(ac$bca[5], dig), "$)^{[a]}$")
        if (ac$bca[4] * ac$bca[5] <= 0) 
        ...
        greg = "r"
        calcIC = ""
        IC = paste0("(", round(a$conf.int[1], dig), " ,", 
            round(a$conf.int[2], dig), ")")
    ...
    if (greg == "r") 
        greg2 = "$r^2$"   else greg2 = "${\\rho}^2$"
    coefdet = paste(" Calculamos também o coeficiente de determinação, dado pelo quadrado da correlação ", 
        greg2, "=$(", round(rho, dig), ")^2$=", rho2, 
        ", que indica ", 100 * rho2, "% de variância compartilhada entre ", 
        det, sep = "")
    if (a$estimate < 0) 
        direcao = "inversa entre as variáveis (nos indivíduos em que uma é maior, a outra é menor)." else direcao = "direta entre as variáveis (nos indivíduos em que uma é maior, a outra é também é)."
    ictexto = paste0(" O intervalo de confiança ", IC, 
        calcIC, avalic, " Reiteramos a importância de avaliar o significado deste resultado na prática.", 
        collapse = "")
    if (abs(a$estimate) >= 0.1 & abs(a$estimate) < 0.3) 
        mag = "Cohen(1992) propõe esta magnitude de correlação como fraca."   else 
          if (abs(a$estimate) >= 0.3 & abs(a$estimate) < 0.5) 
        mag = "Cohen(1992) propõe esta magnitude de correlação como moderada." else 
          if (abs(a$estimate) >= 0.5) 
        mag = "Cohen(1992) propõe esta magnitude de correlação como forte." else mag = "Cohen(1992) propõe esta magnitude de correlação como insignificante ou irrisória."
    if (a$p.value < 0.05) {
    ...
    methodprint = method
    if (idioma == "PT") 
        textograf = substitute(paste("Correlação de ", 
            methodprint, " (", rho, "=", rhoprint, 
            ", p-valor=", pvalorprint, ")"), list(rhoprint = rhoprint, 
            pvalorprint = pvalorprint, methodprint = methodprint))   else textograf = substitute(paste(methodprint, " correlation (", 
        rho, "=", rhoprint, ", p=", pvalorprint, 
        ")"), list(rhoprint = rhoprint, pvalorprint = pvalorprint, 
        methodprint = methodprint))
    result <- data.frame(Variável = nomex, `p-valor` = p, 
        rho = rho, rho2 = rho2, IC = IC)
    names(result) = c("Variável", "p-valor", "Estatística", 
        "Variância compartilhada", "IC (95%)")
    if(ordinalx==F & ordinaly==F) grafico = grafico_correl(as.numeric(y), nomey, cor, as.numeric(x), nomex, textograf, idioma) else
